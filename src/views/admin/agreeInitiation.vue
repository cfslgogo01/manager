<template>
  <div style="padding-bottom: 60px;">
    ...
    <el-table ref="multipleTableRef" height="600px" lazy="true" :data="没有第二同意的列表.data" style="width: 100%"
      table-layout="fixed">

      <el-table-column label="序号" type="index" />
      <!-- <el-table-column label="激活信息号" prop="ainfo_num" /> -->
      <el-table-column label="申请管理员" prop="admin_num_one" />
 ...
 * 为 发起 授权，也就是说，
 * 先查询，然后点同意
 * GET /admin/queryAllnotFreeFuncs
 * 然后渲染
 * 然后同意
 */
...
const 更新自己页面的表格 = () => {
  axios.get(myconfig.SERVER_URL + "/admin/querySecondAuthorization").then(
    res => {
      if (res.status == 200) {
...
const 同意一下 = (row) => {
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      axios.get(myconfig.SERVER_URL + "/admin/agreeOthersInitiate?" + myconfig.getParams(params)).then(
        res => {
          if (res.status == 200) {
            ElMessage({
              type: 'success',
