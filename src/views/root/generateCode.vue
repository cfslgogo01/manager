...
axios
  .get(myconfig.SERVER_URL + "/root/queryAllnotFreeFuncs")
  .then((response) => {
...
const 发送获得激活码的请求 = () => {
  ...
  myrequest.root_num = myconfig.getroot_num();
  ...
  axios
    .get(
      myconfig.SERVER_URL + "/root/generateActivationcodeToFuncs?" + queryString
    )
    .then((res) => {
      console.log(res);
      if (res.status == 200) {
